...
namespace ns3{
  ...
  void Sink::DAFU(void)
  {
    ...
    for(uint32_t i = 0; i<serviceN; i++)
    {
      for(uint32_t j = 0; j<service_ssN[i]; j++)
      {
        temp = state[i].sampleValue[j];
        if(temp>min && temp!=0)
        {
          DAFU_target[min_loc] = j;
          min = temp;
          for(int32_t k = 0; k<target_num; k++)
          {
            double temp2 = 0.5;
            if(DAFU_target[k] != -1)
              temp2 = state[i].sampleValue[DAFU_target[k]];
            if(min>temp2)
            {
              min = temp2;
              min_loc = k;
  ...
  void Sink::DAFU_setAction(int32_t* location, uint32_t index)
  {
    int32_t jump = 0;
    uint32_t nodeNum = service_ssN[index];
    uint32_t nodeLen = sqrt(nodeNum);
    int32_t window = 1;
    int32_t* target = new int32_t[service_ssN[index]];
    int32_t target2[5] = {39,57,78,116,124};
    ...
    for(int32_t i = 0; i<target_num; i++)
    {
      ...
      {
        for(int32_t j = -window; j<window+1; j++)
        {
          for(int32_t k = -window; k<window+1; k++)
          {
    ...
    if(targetNum >0)
      FPS_for_target = (avgRate*service_ssN[0]-(nodeNum-targetNum)*FPS_for_target_min-targetNum2*FPS_for_init)/targetNum;
    else
    {
    ...
    {
      if(target[i]==2 || target[i]==-2)
        state[index].action[i] = (uint32_t)FPS_for_target; //#### -> targetThr
      else if(target[i] == -1)
        state[index].action[i] = (uint32_t)FPS_for_init;
      else
